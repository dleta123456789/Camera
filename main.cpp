...
int main()
{
	...
	*/
	...
	//new verices will have vertex and texture coordiantes only
	float vertices[] = {
		//postions of vertex// texture coordiantes 
	...
	//Object list for the objects
	/*
	* These are for the same objects so only need to change to
	*/
	...
	*/
	...
	stbi_set_flip_vertically_on_load(true); // tell stb_image.h to flip loaded texture's.
	...
	*/
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		*/
		...
		/*
		* glm::radians(rotateValue) handles the angle which has to be rototated.
		* glm::vec3(0.0f, 0.0f, 1.0f) handles which x,y,z coordinate has to be rotatated.
		* Currently it is set to only z coordiante as 1.0f.
		*/
		...
		* fovy= the fov of view angles for y direction
		* aspect= is the resoltuion aspect such as 4:3,16:9. It is calculated by simply by dividing the width divided by height
		* near= distance from the viewer to the clipping plane.
		* faar= far to clipping plane
		*/
